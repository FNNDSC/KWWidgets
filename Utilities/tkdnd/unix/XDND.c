...
void XDND_Reset(XDND *dndp) {
    /*
   * If we are in the middle of handling a tcl callback, don't reset the
   * structure, as we may get called from a ClientMessage Handler, due to an
   * "update" tcl statement...
   */
  /* if (dndp->ExecutingCallback) return; */
    ...
    dndp->CursorWindow                = NULL;
    dndp->CursorCallback              = NULL;
...
#ifdef TKDND_ENABLE_MOTIF_DRAGS
    dndp->Motif_ToplevelAware         = False;
    dndp->Motif_LastToplevel          = None;
#endif /* TKDND_ENABLE_MOTIF_DRAGS */
} /* XDND_Reset */

/*
...
/*XDNDCursor *cursor;
  XColor black, white;*/
    ...
    XQueryColor(display,
      DefaultColormap(display, DefaultScreen(display)), &black);
    XQueryColor(display,
      DefaultColormap(display, DefaultScreen(display)), &white);
  ...
  cursor->image_pixmap = cursor->mask_pixmap = 0;
    ...
    dndp->DNDActionPrivateXAtom =
  XInternAtom(display, "XdndActionPrivate", False);
  ...
  XInternAtom(display,"XdndActionDescription", False);
    ...
    dndp->DNDNonProtocolAtom  =
  XInternAtom(display, "TkDndBinarySelectionAtom", False);
...
#endif /* TKDND_ENABLE_MOTIF_DROPS */
#ifdef TKDND_ENABLE_MOTIF_DRAGS
    sprintf(SelectionName, "TK_MOTIF_Selection_%d",(int) getpid());
    dndp->Motif_DND_Selection   = XInternAtom(display, SelectionName, False);
    dndp->Motif_DND_WM_STATE    = XInternAtom(display, "WM_STATE", False);
#endif /* TKDND_ENABLE_MOTIF_DRAGS */
    ...
    /*
     * Try to get the modifier the Alt key is associated with...
     * (Laurent Riesterer 24/06/2000, taken from tkUnixKey.c)
     */
    dndp->Alt_ModifierMask  = 0;
    dndp->Meta_ModifierMask = 0;
    ModMap  = XGetModifierMapping(dndp->display);
    codePtr = ModMap->modifiermap;
    max     = 8 * ModMap->max_keypermod;
    for (i = 0; i < max; i++, codePtr++) {
  if (*codePtr == 0) continue;
  keysym = XKeycodeToKeysym(dndp->display, *codePtr, 0);
  if ((keysym == XK_Alt_L) || (keysym == XK_Alt_R)) {
      dndp->Alt_ModifierMask  |= ShiftMask << (i/ModMap->max_keypermod);
  }
  if ((keysym == XK_Meta_L) || (keysym == XK_Meta_R)) {
      dndp->Meta_ModifierMask |= ShiftMask << (i/ModMap->max_keypermod);
  }
    }
...
void XDND_Enable(XDND *dnd, Window window) {
    ...
    /*
     * Find the real toplevel that holds the wanted window. The property must
     * be set on each top-level X window that contains widgets that can accept
    ...
    if (status && dnd->WidgetExistsCallback) {
  if (!(*dnd->WidgetExistsCallback) (dnd, parent)) {
      ...
      XChangeProperty(dnd->display, window, dnd->DNDAwareXAtom,
...
XDND_BOOL XDND_IsDndAware(XDND *dnd, Window window, Window* proxy, Atom *vers) {
  ...
  *proxy = (Window) rawData;
  ...
  if (data[0] >= XDND_MINVERSION) {
      ...
      XDND_DEBUG3("XDND_IsDndAware: "
                        "Window %ld is XDND aware. Version: %ld\n",
             window, *vers);
  } else {
      *proxy = None;
  }
...
void XDND_AnnounceTypeList(XDND *dnd, Window window, Atom *list) {
    ...
    if (toplevel != None) {
  XDND_DEBUG2("XDND_AnnounceTypeList: "
                    "Refresing XdndAware on toplevel %ld\n", toplevel);
  XChangeProperty(dnd->display, toplevel, dnd->DNDAwareXAtom,
    XA_ATOM, 32, PropModeReplace, (unsigned char *) &version, 1);
  XChangeProperty(dnd->display, toplevel, dnd->DNDAwareXAtom,
    XA_ATOM, 32, PropModeAppend,  (unsigned char *) list, 
    (typeCount > 3 ? 3 : typeCount));
#ifdef TKDND_ENABLE_MOTIF_DRAGS
  DndWriteSourceProperty(dnd->display, toplevel,
    dnd->Motif_DND_Selection, list, typeCount);
#endif /* TKDND_ENABLE_MOTIF_DRAGS */
    }
   ...
   */
  if (toplevel != None) {
      XChangeProperty(dnd->display, toplevel, dnd->DNDTypeListXAtom,
        XA_ATOM, 32, PropModeReplace, (unsigned char*) list,
        typeCount);
  }
  XChangeProperty(dnd->display, window, dnd->DNDTypeListXAtom,
    XA_ATOM, 32, PropModeReplace, (unsigned char*) list,
    typeCount);
  ...
  AtomName = XGetAtomName(dnd->display, list[i]);
...
void XDND_AnnounceAskActions(XDND *dnd, Window window, Atom *Actions,
  ...
  XChangeProperty(dnd->display, toplevel, dnd->DNDActionListXAtom,
  ...
  AtomName = XGetAtomName(dnd->display, Actions[i]);
  XDND_DEBUG3("\t==> Atom[%d]: %s\n", i, AtomName);
  XFree(AtomName);
    }
#endif /* DND_DEBUG */
...
Atom *XDND_GetAskActions(XDND *dnd, Window window) {
  ...
  if (rawData != NULL) XFree(rawData);
  /*
   * In this case, we simulate one by creating a "list" with only one
   * entry, the desired by the dragger action...
   */
  if (dnd->SupportedAction != None) {
      ActionList = (Atom *) Tcl_Alloc(sizeof(Atom)*2);
      if (ActionList == NULL) return NULL;
      ActionList[0] = dnd->SupportedAction;
      ActionList[1] = None;
  }
    }
    return ActionList;
...
char *XDND_GetAskActionDescriptions(XDND *dnd, Window window) {
  ...
  if (dnd->DraggerAskDescriptions != NULL) {
      memset(dnd->DraggerAskDescriptions, '\0',
...
XDND_BOOL XDND_SendDNDStatus(XDND *dnd, Atom action) {
  ...
  return False;
    ...
    /*
     * Placing the widget the mouse is in was a bug, reported by
     *
...
int XDND_HandleDNDStatus(XDND *dnd, XClientMessageEvent clientMessage) {
  ...
  /*
   * We got None as a supported action. This means that the drop will
   * not be accepted...
   */
  ...
  /*
   * TODO: I have decided to ignore the None value, because there are a
   * few malbehaved applications, like cooledit that they send None but
   * they mean drop :-)
   */
  ...
  if (dnd->SetCursor != NULL) {
    ...
    XDND_DEBUG2("XDND_HandleDNDStatus: ClientMessage from window %ld\n",
      target);
    if (TargetSupportedAction == None) {
  XDND_DEBUG4("    Accepts Drop=%d, Wants Position=%d, Action=%s,\n",
                TargetWillAccept, TargetWantsPosition, "None");
    } else {
  AtomName = XGetAtomName(dnd->display, TargetSupportedAction);
  XDND_DEBUG4("    Accepts Drop=%d, Wants Position=%d, Action=%s,\n",
                TargetWillAccept, TargetWantsPosition, AtomName);
  XFree(AtomName);
    }
  ...
  *version  = None;
  ...
  if (aware != NULL && !(*aware)) {
    ...
    *aware = True;
    ...
    /*
     * If Motif drops are allowed, check for the "WM_STATE"
     * property.  This should be defined only on toplevels,
     * according to Motif...
     */
    XGetWindowProperty(dnd->display, child,
      dnd->Motif_DND_WM_STATE,
      0, 0, False, AnyPropertyType,
      &type, &format, &nitems, &after, &data);
    if (type != None)  *toplevel = child;
...
XDND_BOOL XDND_BeginDrag(XDND *dnd, Window source, Atom *actions, Atom *types,
  ...
  char *cursor_callback) {
    ...
    Atom actualAction;
    Time event_time;
    int accept, win_x_temp, win_y_temp, ret;
    int status = True, want_position;
    unsigned int mask_return;
    float x_mouse, y_mouse;
#ifdef TKDND_ENABLE_MOTIF_DRAGS
    XClientMessageEvent cm;
    int Motif_do_messaging = False, check_motif = True;
    unsigned char receiv_protocol_style = DND_DRAG_NONE;
#endif /* TKDND_ENABLE_MOTIF_DRAGS */
  ...
  if (xevent.type == ButtonRelease) {
    ...
    dnd->button = (xevent.xmotion.state & Button1Mask ? 1 :
      xevent.xmotion.state & Button2Mask ? 2 :
      xevent.xmotion.state & Button3Mask ? 3 :
      xevent.xmotion.state & Button4Mask ? 4 : 5);
    dnd->state  = 0;
    XDND_DEBUG2("XDND_BeginDrag: dragging with button: %d\n", dnd->button);
    ...
    dnd->CursorCallback         = cursor_callback; /* TODO: cache string! */
    ...
    dnd->index                  = 0;
    /* Reset Cursor Cache... */
    if (dnd->SetCursor != NULL) (*dnd->SetCursor) (dnd, -2);
    ...
    XSetSelectionOwner(dnd->display, dnd->DNDSelectionName, dnd->DraggerWindow,
      CurrentTime);
#ifdef TKDND_ENABLE_MOTIF_DRAGS
    XSetSelectionOwner(dnd->display, dnd->Motif_DND_Selection,
    ...
    if (XGrabPointer(dnd->display, dnd->RootWindow, False,
      ...
      GrabModeAsync, GrabModeAsync, None, dnd->cursors[0].cursor,
      CurrentTime) != GrabSuccess) {
        XDND_DEBUG("XDND_BeginDrag: Unable to grab pointer\n");
    }
  
    /* 
     * Enter our private Drag Event Loop. We have to by-pass tk's event loop,
     * as intergrating this stuff with tk event loop would require a
     * substantially number of changes and the tkDND would never be a trully
     * loadable extension...
     */
  ...
  switch (xevent.type) {
    ...
    break;
      ...
      /*
       * This event is not actually reported, so we find out by
       * ourselves from motion events
       */
    ...
    /*
     * D R O P S E C T I O N: We have a button release. It is
     * either a drop, or time to exit or private event loop and
     * give control to Tk...
     */
    dnd->state = xevent.xbutton.state; /* Laurent Riesterer 24/06/2000 */
    if (dnd->WillAcceptDropFlag) {
        if (dnd->InternalDrag) {
      if (dnd->data != NULL) Tcl_Free(dnd->data);
      ...
      if (!dnd->InternalDrag) {
          ...
          (*dnd->WidgetApplyLeaveCallback) (dnd,
            dnd->MouseWindow);
      }
        }
        XDND_DEBUG("XDND_BeginDrag: ButtonRelease: exiting tkDND "
          "private event loop without dropping...\n");
    }
    break;
      } /* ButtonRelease */
      /*
       * A key has been pressed. We have to check whether it is a
       * modifier or Escape.
       */
      case KeyPress: {
    sym = XKeycodeToKeysym(dnd->display, xevent.xkey.keycode, 0);
    XDND_DEBUG4("XDND_BeginDrag: KeyPress: state=0x%08x keycode=%d"
      " keysym=%ld\n",
      xevent.xkey.state, xevent.xkey.keycode, sym);
    dnd->state = xevent.xkey.state;
    /*
     * See if the button pressed is a modifier...
     * Laurent Riesterer 09/07/2000:
     *  optimization in setting the mask
     */
    if (       sym == XK_Shift_L   || sym == XK_Shift_R  ) {
        dnd->state |= ShiftMask;
    } else if (sym == XK_Control_L || sym == XK_Control_R) {
        dnd->state |= ControlMask;
    } else if (sym == XK_Meta_L    || sym == XK_Meta_R   ) {
        dnd->state |= dnd->Meta_ModifierMask;
    } else if (sym == XK_Alt_L     || sym == XK_Alt_R    ) {
        dnd->state |= dnd->Alt_ModifierMask;
    }
    xevent.type          = MotionNotify;
    xevent.xmotion.type  = MotionNotify;
    xevent.xmotion.state = dnd->state;
    XSendEvent(dnd->display, xevent.xkey.window,
      False, 0, &xevent);
    break;
      } /* KeyPress */
      /*
       * A key has been released. We have to check whether it is a
       * modifier or Escape...
       */
      case KeyRelease: {
    sym = XKeycodeToKeysym(dnd->display, xevent.xkey.keycode, 0);
    XDND_DEBUG4("XDND_BeginDrag: KeyRelease: state=0x%08x "
      "keycode=%d keysym=%ld\n",
      xevent.xkey.state, xevent.xkey.keycode, sym);
    dnd->state = xevent.xkey.state;
     ...
     * See if the button pressed is a modifier...
     * Laurent Riesterer 09/07/2000:
     *  correction in reseting the mask
     */
    if (       sym == XK_Shift_L   || sym == XK_Shift_R) {
        dnd->state &= ~ShiftMask;
    } else if (sym == XK_Control_L || sym == XK_Control_R) {
    ...
    if (sym == XK_Escape) {
        /*
         * Time to forget the whole insident...
         * Simulate a button release, but with drop denial...
         */
        dnd->WillAcceptDropFlag    = False;
        fake_xevent.type           = ButtonRelease;
        fake_xevent.xbutton.type   = ButtonRelease;
        fake_xevent.xbutton.x      = xevent.xkey.x;
        fake_xevent.xbutton.y      = xevent.xkey.y;
        fake_xevent.xbutton.x_root = xevent.xkey.x_root;
        fake_xevent.xbutton.y_root = xevent.xkey.y_root;
        fake_xevent.xbutton.button = dnd->button;
        fake_xevent.xbutton.state  = xevent.xkey.state;
        XSendEvent(dnd->display, xevent.xkey.window,
          False, 0, &fake_xevent);
    } else {
        /*
         * Simulate a MotionNotify event, so as to call the
         * callbacks again...
         */
        fake_xevent.type          = MotionNotify;
        fake_xevent.xmotion.type  = MotionNotify;
        fake_xevent.xmotion.state = dnd->state;
        XSendEvent(dnd->display, xevent.xkey.window,
          False, 0, &fake_xevent);
    }
    break;
       ...
       */
      case MotionNotify: {
    event_time = xevent.xmotion.time;
    dnd->state = xevent.xmotion.state; /* Laurent Riesterer 24/06/2000 */
    memcpy (&fake_xevent, &xevent, sizeof(xevent));
    XDND_DEBUG2("XDND_BeginDrag: MotionNotify recieved: (state=0x%08x)\n",
      xevent.xmotion.state);
    if (dnd->DraggerWindow == None) goto exit;
    /*
     * Get mouse coordinates relative to the root window.
     */
    if (!XQueryPointer(dnd->display, dnd->DraggerWindow,
      &(dnd->RootWindow),
      &mouseWindow, &(dnd->x), &(dnd->y),
      &win_x_temp, &win_y_temp, &mask_return)) break;
    /*
     * Find the topmost window the mouse is in. The
     * XDND_FindTarget function will search the whole window tree
     * from the root window until the leaf window the mouse is
     * in. If in the way a window that has the XdndAware property
     * is found, the proper slots are filled...
     */
    ...
    /*
     * Have we entered a new toplevel window?  A toplevel window
     * has the XdndAware atom in its properties.  According to the
     * XDND protocol, this is a toplevel window, as only toplevel
     * windows are allowed to carry this property...
     */
    if (dnd->Toplevel != toplevel) {
...
#ifdef TKDND_ENABLE_MOTIF_DRAGS
        /*
         * Send a DND_TOP_LEVEL_LEAVE to the previous Motif
         * toplevel, if any...
         */
        if (dnd->Motif_LastToplevel != None) {
      if (dnd->Motif_DND && Motif_do_messaging) {
          dnd->Motif_DND_Data.reason     = DND_TOP_LEVEL_LEAVE;
          dnd->Motif_DND_Data.time       = event_time;
          dnd->Motif_DND_Data.src_window = dnd->Motif_LastToplevel;
          DndFillClientMessage (dnd->display,
            dnd->Motif_LastToplevel,
                                    &cm, &(dnd->Motif_DND_Data), 0);

          printf("MotifDrag: Sending DND_TOP_LEVEL_LEAVE to window %ld\n",
            dnd->Motif_LastToplevel);
          XSendEvent(dnd->display, dnd->Motif_LastToplevel,
            False, 0, (XEvent *) &cm);
          Motif_do_messaging      = False;
          dnd->Motif_LastToplevel = None;
      }
        }
        /*
         * Is the new toplevel our window?
         * If it is, forget the whole Motif theme...
         */
        check_motif = True;
        if (dnd->WidgetExistsCallback != NULL) {
      if ((*dnd->WidgetExistsCallback) (dnd, toplevel)) {
          check_motif = False;    
      }
        }
        /*
         * Is it a foreign window and Motif DND aware?
         */
        if (toplevel != None && check_motif) {
      printf("MotifDrag: Checking if toplevel %ld is Motif aware...\n",
        toplevel);
      DndReadReceiverProperty(dnd->display, toplevel, 
        &receiv_protocol_style);
        } else {
      receiv_protocol_style = DND_DRAG_NONE;
        }

        if (receiv_protocol_style == DND_DRAG_NONE) {
      dnd->Motif_DND           = False;
      dnd->Motif_ToplevelAware = False;
      Motif_do_messaging       = False;
        } else if (receiv_protocol_style == DND_DRAG_DROP_ONLY) {
      dnd->Motif_DND           = True;
      dnd->Motif_ToplevelAware = True;
      Motif_do_messaging       = False;
      /*ChangeCursorValid(event.xcrossing.display, 
        event.xcrossing.time); */
        } else if (receiv_protocol_style == DND_DRAG_DYNAMIC) {
      /*
       * The cursor must be changed if we enter a drop site.
       */
      ...
      printf("MotifDrag: Sending DND_TOP_LEVEL_ENTER to window %ld\n",
        toplevel);
      dnd->Motif_DND_Data.reason     = DND_TOP_LEVEL_ENTER;
      dnd->Motif_DND_Data.time       = event_time;
      dnd->Motif_DND_Data.src_window = toplevel;
      dnd->Motif_DND_Data.property   = dnd->Motif_DND_Selection;

      DndFillClientMessage (dnd->display, toplevel,
        &cm, &(dnd->Motif_DND_Data), 0);

      XSendEvent(dnd->display, toplevel, False, 0, (XEvent *)&cm);
      dnd->Motif_DND           = True;
      dnd->Motif_ToplevelAware = True;
      dnd->Motif_LastToplevel  = toplevel; 
      Motif_do_messaging       = True ;
        }
#endif /* TKDND_ENABLE_MOTIF_DRAGS */
    ...
    if (dnd->MouseWindow != mouseWindow) {
      ...
      XDND_DEBUG("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@\n");
        ...
        if (dnd->MouseWindow != None) {
      /*
       * Was the last window one of our own windows? If this
       * is the case, call the callbacks directly. There is
       * no need to go through the XServer...
       */
      if (dnd->InternalDrag) {
          XDND_DEBUG2("XDND_BeginDrag: Internal Window, executing Leave "
            "directly for window %ld\n", dnd->MouseWindow);
        ...
        if (status == TCL_ERROR) {
          ...
          /*
           * The last window belongs to a foreign
           * application. Send the proper messages through
           * the XServer...
           */
      ...
      dnd->SupportedAction = actions[0];
        ...
        dnd->MsgWindow = msgWindow;
        /*
         * We are now officially in the new window. Is the new
         * window XDND aware?
         */
        if (dnd->MouseWindowIsAware) {
      ...
      if (dnd->WidgetExistsCallback != NULL) {
        ...
        /*
         * This is an internal window: Do not send a
         * XdndEnter message through the XServer, but
        ...
        if (dnd->WidgetApplyEnterCallback != NULL) {
            accept = (*dnd->WidgetApplyEnterCallback) (dnd, mouseWindow,
              ...
              event_time, dnd->DraggerTypeList);
            if (dnd->CallbackStatus != TCL_OK) goto exit;
#ifdef XDND_DEBUG
            if (!accept) XDND_DEBUG("XDND_BeginDrag: Enter denied!\n");
#endif /* XDND_DEBUG */
            if (dnd->SupportedAction == None) break;
        } /* if (dnd->WidgetApplyEnterCallback != NULL) */
          } /* if ((*dnd->WidgetExistsCallback) (dnd, mouseWindow)) */
      } /* if (dnd->WidgetExistsCallback != NULL) */
            
      /*
       * If the new window does not belong to this
       * application, send the XdndEnter message...
       */
      if (!dnd->InternalDrag) {
          XDND_DEBUG2("XDND_BeginDrag: Foreign Window, sending XdndEnter "
            "ClientMessage to window %ld\n",
            dnd->MsgWindow);
          XDND_SendDNDEnter(dnd, mouseWindow,
            dnd->MsgWindow, True, dnd->XDNDVersion);
      }
        } else {
      /*
      ...
      XDND_DEBUG2("XDND_BeginDrag: Window %ld is not XDND aware...\n",
        dnd->Toplevel);
      if (dnd->SetCursor != NULL) {
          ret = (*dnd->SetCursor) (dnd, XDND_NODROP_CURSOR);
          if (!ret) goto exit;
      }
      dnd->MouseWindowIsAware = False;
      dnd->MsgWindow = None;
    ...
    /*
     * Whether we are in a new window or not, report the mouse
     * position to the window, if it is XDND aware...
     */
    if (dnd->MouseWindowIsAware && dnd->SupportedAction != None) {
        if (dnd->InternalDrag) {
      /*
       * Simulate the XdndPosition/XdndStatus dialog without
       * going through the X-Server, as it is an internal
       * drop...
       */
      dnd->WaitForStatusFlag = False;
      if (dnd->WidgetApplyPositionCallback != NULL) {
          status = (*dnd->WidgetApplyPositionCallback) (dnd, 
            ...
            event_time, dnd->DraggerTypeList,
            &want_position, &(dnd->SupportedAction),
            &(dnd->DesiredType), &rectangle);
        ...
        /*
         * Change the cursor according to the
         * supported by the target action...
         */
        if (dnd->SetCursor != NULL) {
            if (dnd->SupportedAction == dnd->DNDActionCopyXAtom) {
          ret = (*dnd->SetCursor) (dnd, XDND_COPY_CURSOR);
            } else if (dnd->SupportedAction == dnd->DNDActionMoveXAtom) {
          ret = (*dnd->SetCursor) (dnd, XDND_MOVE_CURSOR);
            } else if (dnd->SupportedAction == dnd->DNDActionLinkXAtom) {
          ret = (*dnd->SetCursor) (dnd, XDND_LINK_CURSOR);
            } else if (dnd->SupportedAction == dnd->DNDActionAskXAtom) {
          ret = (*dnd->SetCursor) (dnd, XDND_ASK_CURSOR);
            } else if (dnd->SupportedAction == dnd->DNDActionPrivateXAtom) {
          ret = (*dnd->SetCursor) (dnd, XDND_PRIVATE_CURSOR);
            } else {
          ret = (*dnd->SetCursor) (dnd, XDND_NODROP_CURSOR);
            }
            if (!ret) goto exit;
      ...
      /*
       * This is a foreign window. Send XdndPosition
       * message...  */
      if (!dnd->WaitForStatusFlag) {
          /*
           * If we are not waiting for XdndStatus message
           * (in a slow network...) send an XdndPosition
           * message:
           */
        ...
        if (check_motif && dnd->Motif_DND && Motif_do_messaging) {
      printf("MotifDrag: Sending DND_DRAG_MOTION to window %ld\n",
        dnd->Motif_LastToplevel);
      dnd->Motif_DND_Data.reason     = DND_DRAG_MOTION;
      dnd->Motif_DND_Data.time       = event_time;
      /* TODO: Proper support for motif actions... */
      dnd->Motif_DND_Data.operation  = DND_COPY;          /* TODO */
      dnd->Motif_DND_Data.operations = DND_MOVE|DND_COPY; /* TODO */
      dnd->Motif_DND_Data.x          = dnd->x;
      dnd->Motif_DND_Data.y          = dnd->y;

      DndFillClientMessage (dnd->display, dnd->Motif_LastToplevel,
        &cm, &(dnd->Motif_DND_Data), 0);

      XSendEvent(dnd->display, dnd->Motif_LastToplevel,
        False, 0, (XEvent *)&cm);
      /* TODO: Continue with motif drag :-) */
        }
#endif /* TKDND_ENABLE_MOTIF_DRAGS */
    } /* if (dnd->MouseWindowIsAware) */
    /* 
     * The following call will not change the mouse cursor.
     * It will only move the cursor window, only if its position is not
     * the current one (stored internally).
     */
    if (dnd->SetCursor != NULL) {
        ret = (*dnd->SetCursor) (dnd, -1);
        if (!ret) goto exit;
    }
    break;
      } /* MotionNotify */
    ...
    break;
      ...
      case SelectionRequest: {
    XDND_DEBUG("XDND_BeginDrag: "
      "SelectionRequest: getting widget data\n");
    if (xevent.xselectionrequest.selection
      != dnd->DNDSelectionName) break;
    /*
     * Retieve the data from widget...
...
int XDND_HandleDNDEnter(XDND *dnd, XClientMessageEvent clientMessage) {
    ...
    /*
   * We should not receive such an event during a drag operation...
   */
    if (dnd->IsDraggingFlag) {
  XDND_DEBUG("XDND_HandleDNDEnter: Received during drag?\n");
  return False;
    }
  
    vers = (clientMessage.data.l[1] >> 24);
    XDND_DEBUG2("XDND_HandleDNDEnter: Requesting protocol version %d\n", vers);
  ...
  typelist[3] = None;
#ifdef DND_DEBUG
  XDND_DEBUG("XDND_HandleDNDEnter: "
    "Drag source supports at most 3 types:\n");
  ...
  if (typelist == NULL) {
      XDND_DEBUG("XDND_HandleDNDEnter: "
        "There are no supported types by the drag source!\n");
      return False;
  }
#ifdef DND_DEBUG
  XDND_DEBUG("XDND_HandleDNDEnter: "
    "Drag source supports more than 3 types\n");
  for (atom = typelist; *atom != 0; atom++) {
      AtomName = XGetAtomName(dnd->display, *atom);
      XDND_DEBUG2("       %s\n", AtomName);
      XFree(AtomName);
...
int XDND_HandleDNDHere(XDND *dnd, XClientMessageEvent clientMessage) {
   ...
   */
    if (dnd->IsDraggingFlag) {
  XDND_DEBUG("XDND_HandleDNDHere: Received during drag?\n");
  return False;
    }
  
    if (dnd->DraggerWindow != (Window) clientMessage.data.l[0]) {
  XDND_DEBUG("XDND_HandleDNDHere: bad DraggerWindow\n");
  return False;
    }
    ...
    if (!XDND_FindTarget(dnd, dnd->x, dnd->y, NULL, NULL, &target,
      NULL, NULL)) {
  XDND_DEBUG("XDND_HandleDNDHere: target NOT found\n");
  return False;
    }
    XDND_DEBUG("XDND_HandleDNDHere: target found\n");
  ...
  if (dnd->MouseWindow != None
    && dnd->WidgetApplyLeaveCallback != NULL) {
      (*dnd->WidgetApplyLeaveCallback) (dnd, dnd->MouseWindow);
  }
  dnd->MouseWindow = target;
  if (dnd->WidgetApplyEnterCallback != NULL) {
      will_accept = (*dnd->WidgetApplyEnterCallback) (dnd, target,
        dnd->DraggerWindow, action, dnd->x, dnd->y, time,
        dnd->DraggerTypeList);
    ...
    if (will_accept) {
  if (dnd->DraggerAskActionList == NULL) {
      dnd->DraggerAskActionList = XDND_GetAskActions(dnd,
        dnd->DraggerWindow);
      XDND_GetAskActionDescriptions(dnd, dnd->DraggerWindow);
      /*
       * If we fail, make a "list" out of the Dragger supported action...
       */
      dnd->DraggerAskActionList = (Atom *) Tcl_Alloc(sizeof(Atom)*2);
      if (dnd->DraggerAskActionList != NULL) {
    dnd->DraggerAskActionList[0] = action;
    dnd->DraggerAskActionList[1] = None;
      }
  }
  dnd->WillAcceptDropFlag =
      (*dnd->WidgetApplyPositionCallback) (dnd, target, 
        dnd->DraggerWindow, action, dnd->DraggerAskActionList,
        dnd->x, dnd->y, time, dnd->DraggerTypeList, &wantPosition,
        &supportedAction, &desired_type, &rectangle);
  /* TODO: The callback may raise an error. What can we do? The drag
   * source has a global cursor drag... */
  dnd->DesiredType = desired_type;
#ifdef DND_DEBUG
  if (dnd->WillAcceptDropFlag) {
      AtomName = XGetAtomName(dnd->display, supportedAction);
...
int XDND_HandleDNDLeave(XDND *dnd, XClientMessageEvent clientMessage) {
   ...
   */
    if (dnd->IsDraggingFlag) {
  XDND_DEBUG("XDND_HandleDNDLeave: Received during drag?\n");
  return False;
    }
  
    /*XSelectInput(dnd->display, dnd->DraggerWindow, NoEventMask);*/
    if (dnd->DraggerWindow != clientMessage.data.l[0]) {
  XDND_DEBUG("XDND_HandleDNDLeave: bad DraggerWindow\n");
  return False;
    }

    /*
   * Declare that we want to reset values. If we execute a tcl callback that
   * contains an "update" command, it is very possible that a ClientMessage is
   * handled, handling an XdndEnter/XdndPosition event. We don't want to loose
   * them :-)...
   */
    dnd->ResetValues = True;
...
int XDND_HandleDNDDrop(XDND *dnd, XClientMessageEvent clientMessage) {
   ...
   */
    if (dnd->IsDraggingFlag) {
  XDND_DEBUG("XDND_HandleDNDDrop: Received during drag?\n");
  return False;
    }
  
    if (dnd->DraggerWindow != (Window) clientMessage.data.l[0]) {
  XDND_DEBUG("XDND_HandleDNDDrop: bad DraggerWindow\n");
  dnd->DraggerWindow = (Window) clientMessage.data.l[0];
    }
    if ((tkwin = Tk_IdToWindow(dnd->display, dnd->MouseWindow)) == NULL) {
  XDND_DEBUG("XDND_HandleDNDDrop: bad Mouse Window\n");
  goto finish;
    }
  ...
  if (dnd->data != NULL) Tcl_Free(dnd->data);
  dnd->data = NULL; dnd->index = 0;
  selectionOwner = XGetSelectionOwner(dnd->display,
    dnd->DNDSelectionName);
  if (selectionOwner == None) goto finish;
#ifdef DND_DEBUG
  XDND_DEBUG3("<Drop>: Getting selection with type \"%s\" (0x%08x)\n",
                Tk_GetAtomName(tkwin, dnd->DesiredType), dnd->DesiredType);
#endif
  XConvertSelection(dnd->display, dnd->DNDSelectionName,
    dnd->DesiredType, dnd->DNDNonProtocolAtom, dnd->MouseWindow,
    CurrentTime);
  startTime = clock();
  endTime   = startTime + 5 * CLOCKS_PER_SEC;
  while (!receivedEvent && clock() < endTime) {
      receivedEvent = XCheckTypedWindowEvent(dnd->display,
        dnd->MouseWindow,
  ...
  do {
    ...
    goto finish;
      ...
      read += count;
      if (data == NULL) {
    data = (unsigned char *) Tcl_Alloc(sizeof(unsigned char)
      *(read+2));
    dnd->index = 0;
  ...
  (*dnd->WidgetInsertDropDataCallback) (dnd, (unsigned char *) dnd->data,
    dnd->index, 0, dnd->MouseWindow, dnd->DraggerWindow,
    dnd->DesiredType);
    }
    if (dnd->data != NULL) Tcl_Free(dnd->data);
    dnd->data = NULL; dnd->index = 0;
