...
vtkCxxRevisionMacro(vtkKWScale, "$Revision: 1.102 $");
...
vtkKWScale::vtkKWScale()
{
  ...
  this->Scale           = vtkKWCoreWidget::New();
  this->Entry           = NULL;
  this->Label           = NULL;
  this->TopLevel        = NULL;
...
void vtkKWScale::Create(vtkKWApplication *app)
{
  ...
  this->Scale->SetConfigurationOption("-orient", "horizontal");
  this->Scale->SetConfigurationOption("-showvalue", "no");
  this->Scale->SetBorderWidth(2);
  this->Scale->SetHighlightThickness(0);
  this->Scale->SetConfigurationOptionAsDouble("-resolution", this->Resolution);
  this->Scale->SetConfigurationOptionAsDouble("-from", this->Range[0]);
  this->Scale->SetConfigurationOptionAsDouble("-to", this->Range[1]);
...
{
  this->SetBinding("<Configure>", this, "Resize");
  
  if (this->Scale && this->Scale->IsCreated())
    {
    this->Scale->SetBinding(
      "<ButtonPress>", this, "InvokeStartCommand");
    this->Scale->SetBinding(
      "<ButtonRelease>", this, "InvokeEndCommand");

    this->Scale->AddBinding(
      "<ButtonPress>", this, "DisableScaleValueCallbackOff");
    this->Scale->AddBinding(
      "<ButtonRelease>", this, "DisableScaleValueCallbackOn");
    ...
    if (this->PopupScale &&
      ...
      this->TopLevel->SetBinding(
        "<Leave>", this, "WithdrawPopupScaleCallback");
      ...
      callback += " WithdrawPopupScaleCallback";
    ...
    this->Scale->SetConfigurationOption("-command", command);
    delete [] command;
    }
  ...
  if (this->PopupScale && 
    ...
    this->PopupPushButton->SetBinding(
      "<ButtonPress>", this, "DisplayPopupScaleCallback");
    }
}
...
void vtkKWScale::UnBind()
{
  ...
  if (this->Scale && this->Scale->IsCreated())
    {
    this->Scale->RemoveBinding("<ButtonPress>");
    this->Scale->RemoveBinding("<ButtonRelease>");
    this->Scale->SetConfigurationOption("-command", NULL);
    }
...
void vtkKWScale::SetResolution(double r)
{
  ...
  if (this->Scale && this->Scale->IsCreated())
    {
    this->Scale->SetConfigurationOptionAsDouble("-resolution", r);
    }
}
...
void vtkKWScale::RefreshValue()
{
  if (this->Scale && this->Scale->IsCreated())
    ...
    if (was_disabled)
      {
      this->Scale->SetState(vtkKWTkOptions::StateNormal);
    ...
    if (was_disabled)
      {
      this->Scale->SetState(vtkKWTkOptions::StateDisabled);
...
void vtkKWScale::SetRange(double min, double max)
{
  ...
  if (this->Scale)
    {
    this->Scale->SetConfigurationOptionAsDouble("-from", min);
    this->Scale->SetConfigurationOptionAsDouble("-to", max);
    }
  if (this->RangeMinLabel)
    {
    this->RangeMinLabel->SetConfigurationOptionAsDouble("-text", min);
    }
  if (this->RangeMaxLabel)
    {
    this->RangeMaxLabel->SetConfigurationOptionAsDouble("-text", max);
    }
}
...
// ---------------------------------------------------------------------------
void vtkKWScale::SetEntryCommand(vtkObject *object, const char * method)
{
  this->SetObjectMethodCommand(
...
void vtkKWScale::UpdateEnableState()
{
  ...
  if (this->Scale)
    {
    this->Scale->SetState(this->GetEnabled());
    }
...
void vtkKWScale::Resize()
{
  ...
  if (width < this->ShortWidth)
    ...
    if (this->RangeMinLabel)
      {
      this->RangeMinLabel->SetConfigurationOption("-text", NULL);
      }
    if (this->RangeMaxLabel)
      {
      this->RangeMaxLabel->SetConfigurationOption("-text", NULL);
      }
    this->Scale->SetConfigurationOptionAsInt("-sliderlength", 10);
    ...
    {
    this->Scale->SetConfigurationOptionAsInt("-sliderlength", 15);
    this->PackRange = 0;
    this->PackEntry = 0;
    this->PackWidget();
    }
    ...
    {
    this->Scale->SetConfigurationOptionAsInt("-sliderlength", 20);
    ...
    {
    this->Scale->SetConfigurationOptionAsInt("-sliderlength", 25);
    ...
    {
    this->Scale->SetConfigurationOptionAsInt("-sliderlength", 30);
    this->Label->SetText(this->NormalLabel);
    
    if (this->RangeMinLabel && this->RangeMinLabel->IsCreated())
      {
      sprintf(labelText, "(%g)", this->Range[0]);
      this->RangeMinLabel->SetConfigurationOption("-text", labelText);
      }
    if (this->RangeMaxLabel && this->RangeMaxLabel->IsCreated())
      {
      sprintf(labelText, "(%g)", this->Range[1]);
      this->RangeMaxLabel->SetConfigurationOption("-text", labelText);
      }
    this->PackRange = 1;
    this->PackEntry = 1;
    this->PackWidget();
...
void vtkKWScale::SetWidth(int width)
{
  if (this->Scale)
    {
    this->Scale->SetConfigurationOptionAsInt("-width", width);
    }
}
...
{
  this->Scale->SetConfigurationOptionAsInt("-showvalue", val);
}

//----------------------------------------------------------------------------
int vtkKWScale::GetValueVisibility()
{
  return this->Scale->GetConfigurationOptionAsInt("-showvalue");
}
...
void vtkKWScale::SetLength(int length)
{
  if (this->Scale)
    {
    this->Scale->SetConfigurationOptionAsInt("-length", length);
    }
}
