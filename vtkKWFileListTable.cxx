...
vtkCxxRevisionMacro(vtkKWFileListTable, "$Revision: 1.8 $");
...
const char *vtkKWFileListTable::GetNthSelectedFileName(int i)
{
  ...
  if (i < 0 || i >= selrows)
    ...
    vtkErrorMacro(<< this->GetClassName()
                  << " index for GetNthSelectedFileName is out of range");
    return NULL;
    }
  if(selrows > 0)
    {
  ...
  if (this->FileList->GetWidget()->GetNumberOfSelectedRows()>0)
    ...
    dlg->GetEntry()->GetWidget()->SetValue(
      vtksys::SystemTools::GetFilenameName(
       this->GetNthSelectedFileName(0)).c_str());
    dlg->SetText(
    ...
    vtksys_stl::string oldfile = this->GetNthSelectedFileName(0);
    if (rename(this->GetNthSelectedFileName(0), fullname.c_str())==0)
      {
      int *indices = new int [selrows];
      int nb_selected_rows = this->FileList->GetWidget()->
                             GetSelectedRows(indices);
