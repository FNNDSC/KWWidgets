...
vtkCxxRevisionMacro(vtkKWFileListTable, "$Revision: 1.18 $");
  ...
  const char* fileextensions)
{
  ...
  if (tk_foldercmd.rdbuf())
    {
    tk_foldercmd << ends;
    vtkKWTkUtilities::EvaluateSimpleString(
      this->GetApplication(), tk_foldercmd.str());
    }
  ...
  if (tk_filecmd.rdbuf())
    ...
    tk_filecmd << ends;
    vtkKWTkUtilities::EvaluateSimpleString(
      this->GetApplication(), tk_filecmd.str());
    }
  ...
  const char *key)
{
  if (this->FileList->GetWidget()->GetNumberOfSelectedRows() > 0 &&
    ...
    tk_cmd << ends;
    vtkKWTkUtilities::EvaluateSimpleString(
      this->GetApplication(), tk_cmd.str());
